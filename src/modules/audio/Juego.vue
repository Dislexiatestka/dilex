<template>
  ...
  <div class="contenedor">
    <div class="contenedor-puntaje">
      <Img :src="require('@/assets/audio/audio.jpg')" width="125"></Img>
    ...
    />
    <button v-if="intentos == 5" @click="moveToNextWord()" class="btn-siguiente">Siguiente</button>
  </div>
</template>
  ...
  data() {
    return {
      ...
      instruccion: {
        ...
        instruccion:
          "Da clic en el reproductor de audio que tienes en el centro y selecciona la silaba que escuchaste en el audio de las opciones que tienes abajo. ",
      },
    };
  },
  mounted() {
    ...
    while (indicesAleatorios.size < 5) {
      const indiceAleatorio = Math.floor(
        Math.random() * this.wordsGeneral.palabras.length
      );
      indicesAleatorios.add(indiceAleatorio);
    }

    const palabrasAleatorias = Array.from(indicesAleatorios).map(
      (indice) => this.wordsGeneral.palabras[indice]
    );
    this.wordsAleatorias = palabrasAleatorias;
    this.words = palabrasAleatorias.at(0).Opciones.split(",");
    this.audioSrc = require("@/assets/audio/" +
      palabrasAleatorias.at(0).Palabra +
      ".ogg");
  ...
  methods: {
    ...
    moveToNextWord() {
      ...
      if (this.currentIndex < 4) {
        this.currentIndex++;
        this.words = this.wordsAleatorias
          .at(this.currentIndex)
          .Opciones.split(",");
        this.audioSrc = require("@/assets/audio/" +
          this.wordsAleatorias.at(this.currentIndex).Palabra +
          ".ogg");
